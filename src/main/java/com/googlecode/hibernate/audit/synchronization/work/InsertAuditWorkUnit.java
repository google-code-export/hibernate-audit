...
public class InsertAuditWorkUnit extends AbstractAuditWorkUnit {
	...
	public InsertAuditWorkUnit(String entityName, Serializable id,
	...
	private void initializerAuditEvents(Session session,
	...
	private void processProperties(Session session,
		...
		for (String propertyName : persister.getPropertyNames()) {
			...
			processProperty(session, auditConfiguration, auditEvent, entity,
	...
	protected void processEntityProperty(Session session,
		...
		if (propertyValue != null) {
			...
			super.processEntityProperty(session, auditConfiguration, object,
					propertyName, propertyValue, propertyType, auditObject);
		}
	}
	...
	protected void processComponentValue(Session session,
		...
		if (component != null) {
			// only record not null values
			super.processComponentValue(session, auditConfiguration,
					auditEvent, auditObject, entityName, entity, propertyName,
	...
	protected void createSimpleValue(Session session,
		...
		if (propertyValue != null) {
			// only record not null values
			super.createSimpleValue(session, auditConfiguration, auditObject,
					entityName, entity, propertyName, propertyValue);
		}
	}
}
